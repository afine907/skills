...
用户输入：`我的客服 Agent 在长对话中越来越不靠谱`

Claude 输出以上模板，包含 Agent 生命周期图（6 阶段）、追踪 JSON Schema、上下文健康指标表、告警规则矩阵、仪表盘布局描述等。

## 不适用

- 传统 Web 应用 APM → 使用 Datadog / New Relic 等标准 APM 工具
- 非 Agent 系统的日志 → 使用 ELK / Loki 等日志方案
- Prompt 质量评估 → 使用 [agent-eval](../agent-eval/SKILL.md) 技能
