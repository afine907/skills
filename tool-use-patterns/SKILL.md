...
| 工具名 | 风险等级 | 副作用 | 处理方式 |
|--------|---------|--------|---------|
| search_docs | 只读 | 无 | 自动执行 |
| create_issue | 不可逆写入 | 创建 Issue | 显式审批 |
| ... | ... | ... | ... |

### Schema 设计
（展示每个工具的 JSON Schema，包含严格类型和描述）
...
用户输入：`为 GitHub API Agent 设计工具调用方案`

Claude 输出以上模板，包含 GitHub API 工具审计表（search/issues/pull_requests 等）、每个工具的 Schema 设计、错误分类响应策略、工具组合编排图、测试矩阵等。

## 不适用

- 简单无状态 API 调用 → 直接使用 HTTP 客户端（axios / fetch）
- 不需要 Agent 的脚本 → 使用传统编程方式
- Prompt 工程问题 → 使用 prompt-engineering 技能
- Agent 性能监控 → 使用 [llm-observability](../llm-observability/SKILL.md)
